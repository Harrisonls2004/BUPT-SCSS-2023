...
# Destination[i] += i; # 先加 i
# Destination[i] = ((Destination[i] & 3) << 6) | (4 * (Destination[i] & 0xC)) | ((Destination[i] & 0xF0) >> 4) 
# 再进行位操作：最低2位左移6位，中间2位左移4位，最高4位右移4位
...
for i in range(len(s)):
    ...
    for j in range(36, 160):
        # 正向加密逻辑（与 decrypt 对应）
        tmp = j + i  # 先加 i
        tmp = ((tmp & 3) << 6) | (4 * (tmp & 0xC)) | ((tmp & 0xF0) >> 4)
        if tmp == target_byte:
            # 检查是否是可打印 ASCII 字符
            if 32 <= j <= 126:
                ascii_char = chr(j)
